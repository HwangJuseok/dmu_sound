...
public class HomeController {
    ...
    public Mono<String> home(Model model, HttpSession session) { // ✅ 세션 추가
        return spotifyService.getNewReleases()
                .zipWith(youtubeService.getTrendingMusic())
                .map(result -> {
                    List<NewReleaseDto> newReleases = result.getT1();
                    List<TrendingVideoDto> trendingVideos = result.getT2();

                    model.addAttribute("newReleases", newReleases);
                    model.addAttribute("trendingVideos", trendingVideos);

                    // ✅ 세션에 로그인 정보가 있으면 Mustache에 전달
